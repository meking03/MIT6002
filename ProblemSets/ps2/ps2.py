...
#
# Answer:
# Nodes are buildings. Edges are distances. 
...
def load_map(map_filename):
    ...
    for line in file:
        ...
        try:
            school_map.add_node(node1)
        except ValueError:
            pass
        
        try:
            school_map.add_node(node2)
        except ValueError:
            pass
        
        edge = WeightedEdge(node1, node2, int(contents[2]), int(contents[3]))
        school_map.add_edge(edge)
    return school_map
...
#
# Answer: Minimizing total distance travelled while not exceeding total distance outdoors limit.
#
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    # the helper function already returns the shortest path that satisfies the max dist outdoors constraint
    # just have to check max_total_dist
    
    # if the distance returned by the function is equal to or smaller than max total dist
    if best_path != None:
